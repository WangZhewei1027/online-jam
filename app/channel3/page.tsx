...
const Switch: React.FC<SwitchProps> = ({
                                           name, clock, index, player, change, onToggle,
                                       }) => {
    ...
    return (<div
        id="switch"
        className={`flex items-center justify-center ${enabled ? "bg-green-500" : "bg-gray-200"} border border-gray-300 p-2 cursor-pointer w-12 h-14 hover:border-4 rounded-lg`}
...
export default function PlayTone() {
    ...
    useEffect(() => {
        const channel = supabase
            ...
            .on("postgres_changes", {
                event: "UPDATE", schema: "public", table: "notes", filter: "id=eq.4",
            }, (payload) => {
    ...
    return (<>
        {dataLoaded ? <div className="overflow-x-auto">
            <div>
                <div className="grid grid-rows-4 grid-flow-col">
                    {Object.keys(players).map((key) => (<div
                        className="flex justify-center items-center px-4 font-serif"
                        key={key}
                    >
                        {key.charAt(0).toUpperCase() + key.slice(1)}
                    </div>))}
                    {[...Array(64)].map((_, index) => (<div
                    ...
                    >
                        <Switch
                            ...
                            change={json !== null && Object.keys(json).length > 0 ? json[sounds[index % sounds.length]][Math.floor(index / sounds.length)] === 1 : false}
        ...
        </div> : <div className={"flex justify-center items-center mt-8"}><Spinner /></div>}
    </>);
}
